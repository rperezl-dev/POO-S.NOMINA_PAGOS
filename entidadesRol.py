...
class Empleado(ABC): 
    ...
    def mostrarCabeceraRol(self,razonSocial, direccion, telefono, ruc,tipoRol):
        borrarPantalla()
        print('              {} Ruc : {} Teléfono : {} Dirección: {}'.format(razonSocial,ruc,telefono,direccion))
        print('--------------------------------------------------------------------------------------------------------------------')
        print('           N O M I N A   D E   R O L E S  D E   E M P LE A D O : {}  '.format(tipoRol))
        print('Empleado:{}'.format(self.nombre,)," "*10,f"Desde: {self.fechaIngreso}")
        print('Cargo:{}'.format(self.cargo.descripcion))
        print('--'*59) 
        print(" "*5,"R O L E S"," "*37)
        print("Periodo    Sueldo   Sobretiempo  Antiguedad  Comision   TotIng   Iess    Prestamo   TotDes   Neto")

    def mostrarDetalleRol(self):
        fila = 9
        #print(self.detalleNomina)
        for det in self.detalleRol:
            gotoxy(1,fila);print(det[0],end="")
            gotoxy(12,fila);print(det[4],end="")    
            gotoxy(25,fila);print(det[5],end="")          
            gotoxy(37,fila);print(det[6],end="")    
            gotoxy(47,fila);print(det[7],end="")    
            gotoxy(57,fila);print(det[8],end="")    
            gotoxy(65,fila);print(det[9],end="")    
            gotoxy(75,fila);print(det[10],end="")    
            gotoxy(85,fila);print(det[11],end="")    
            gotoxy(95,fila);print(det[12],end="")     
            fila+=1
...
class Obrero(Empleado):
    ...
    def valorAntiguedad(self):
        return round((datetime.now().year - datetime.strptime(self.fechaIngreso, '%Y-%m-%d').year)/365*self.sueldo,2)  
